...
def save_deb_build_result(packagename, logdir, logstdout_filename):
   is_successful = False 
   log_stdout = open(logstdout_filename)
   for line in log_stdout:
       if re.search('dpkg-buildpackage: full upload', line.strip()) or re.search('Successfully signed dsc and changes files', line.strip()):
           is_successful = True 
           break
...
def build_sub_package(buildfile, logdir, succ_dict):
    ...
    try :
        proc = subprocess.check_call([buildfile targetos], stdout=log_stdout, stderr=log_stderr, shell=True)

    except Exception as e:
        print("Catch exception:%s\n"%e)
 
    log_stdout.close()
    save_deb_build_result(packagename, logdir, log_stdout_filename)
    return 0
...
def get_all_build_files(dirname):
    ...
    for filename in os.listdir(dirname):
        ...
        if os.path.isfile(fullname) and filename == 'deb_build.sh':
...
if __name__ == "__main__":
    ...
    print("Please check deb build logs under:%s"%logdir)
    print("Please check deb packages under:~/debbuild/ or /root/debbuild")
