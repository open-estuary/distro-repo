...
def main():	
 ...
 for component in components:
  ...
  if site == 'HDFS':
   XML_DIR=os.path.join(os.path.join(os.environ['HADOOP_ROOT'],sys.argv[1]),"etc/hadoop/hdfs-site.xml")
   doc=xml.dom.minidom.parse(XML_DIR)
   create_kv("ha.zookeeper.quorum",os.environ['zk_address'],doc)
   create_kv("dfs.namenode.name.dir",os.environ['namenode_dfs_name_dir'],doc)
   create_kv("dfs.datanode.data.dir",os.environ['datanode_dfs_data_dir'],doc)
   ...
   XML_DIR=os.path.join(os.path.join(os.environ['HADOOP_ROOT'],sys.argv[1]),"etc/hadoop/core-site.xml")
   doc=xml.dom.minidom.parse(XML_DIR)
   create_kv("hadoop.tmp.dir",os.environ['hadoop_tmp_dir'],doc)
   analysis_xml(component,doc)
   write_xml(XML_DIR,doc)
  elif site =='yarn':
   XML_DIR=os.path.join(os.path.join(os.environ['HADOOP_ROOT'],sys.argv[1]),"etc/hadoop/yarn-site.xml")
   doc=xml.dom.minidom.parse(XML_DIR)
   ...
   XML_DIR=os.path.join(os.path.join(os.environ['HADOOP_ROOT'],sys.argv[1]),"etc/hadoop/mapred-site.xml.template")
   doc=xml.dom.minidom.parse(XML_DIR)
   analysis_xml(component,doc)
