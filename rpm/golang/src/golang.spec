...
# Fedora GOROOT
%global goroot          %{_scl_root}/usr/lib/golang
...
Provides:       %{scl_prefix}go = %{version}-%{release}
...
Requires:       %{scl_prefix}golang = %{version}-%{release}
Requires:       %{scl_prefix}runtime
...
export GOPATH=$(cd ~; pwd)/ububuild/go-output
...
pushd $RPM_BUILD_ROOT%{goroot}/bin > /dev/null
cd linux_%{gohostarch}
ln -sf  ../go    go
ln -sf  ../gofmt gofmt
popd > /dev/null
...
ln -sf  ../lib/golang/bin/go go
ln -sf  ../lib/golang/bin/gofmt gofmt
popd > /dev/null
