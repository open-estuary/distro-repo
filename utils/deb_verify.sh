...
if [ $targetos = "ubuntu" ];then
	docker run -it -v ${logdir}:/tmp/debverifylog -v ${CUR_DIR}/:/root/ --name ${CONTAINER_NAME} openestuary/ubuntu:3.1-full python /root/verify_incontainer.py $cmd /tmp/debverifylog $targetos 
elif [ $targetos = "debian" ];then
	docker run -d -v ${logdir}:/tmp/debverifylog -v ${CUR_DIR}/:/root/ --name ${CONTAINER_NAME} openestuary/debian:3.1-full python /root/verify_incontainer.py $cmd /tmp/debverifylog $targetos
fi
